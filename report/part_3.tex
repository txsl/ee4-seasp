...
In order to derive the leaky LMS equation, we differentiate the given cost function:

\begin{subequations}
\begin{align}
J(n) &= \frac{1}{2} ( e^2(n) + \gamma \lVert \mathbf{w}(n) \rVert ^2 ) \\
\nabla J(n)|_{w_n} &= \frac{1}{2}  \nabla ( ( e^2(n) ) |_{w_n} + \gamma \nabla (\lVert \mathbf{w}(n)\rVert ^2 )|_{w_n} ) \\
